...
// ++ ADDED: Resume from saved phase when navigated from Continue Project button ++
window.addEventListener('DOMContentLoaded', async () => {
  console.log("Dashboard initialized");

  const params          = new URLSearchParams(window.location.search);
  const resumeProjectId = params.get("project_id");
  const resumePhase     = parseInt(params.get("resume_phase"), 10);

  // Only resume if both params are present and phase is in valid range 1-4
  if (resumeProjectId && resumePhase >= 1 && resumePhase <= 4) {
    console.log(`Resuming project ${resumeProjectId} at phase ${resumePhase}`);

    try {
      const res = await fetch(`/projects/load/${encodeURIComponent(resumeProjectId)}`);
      ...
      if (!savedData || savedData.error) {
        throw new Error(savedData?.error || "Project data not found");
      }

      // Hydrate global state — same shape as normal upload flow sets it
      extractedData           = savedData;
      window.currentProjectId = resumeProjectId;

      // Safe fallback to phase 1 if value somehow invalid
      currentPhase = (resumePhase >= 1 && resumePhase <= 4) ? resumePhase : 1;

      // Hide upload step, show pipeline UI directly
      ...
      // Phase 4 needs its own data fetch before rendering
      if (currentPhase === 4) {
        await loadPhase4();
      }
      ...
      // ++ Clean the URL params immediately so any future reload won't re-trigger the resume ++
      history.replaceState(null, "", "/Dashboard");

    } catch (err) {
      console.error("Resume failed — user can start fresh:", err);
    }
  }
...
function renderNonFunctionalRequirements(data) {
  ...
  data.nfr_predictions.forEach((nfr, idx) => {
    html += `
      <div class="mb-3">
        <div class="req-title">${String.fromCharCode(97 + idx)}. ${nfr.predicted_type_label || nfr.title || 'Unknown Type'}</div>
...
async function generateSkeletonWithLanguage() {
  ...
  const generateBtn = document.getElementById("generateSkeletonBtn");
...
async function generateADL() {
  ...
  try {
    ...
    document.getElementById('reportModal').addEventListener(
      'hidden.bs.modal',
...
async function changePhase(dir) {
...
// ++ ADDED: Mark project as fully complete, then navigate to dashboard cleanly ++
async function completeProject() {
  if (!window.currentProjectId) {
    // No project ID — just go to dashboard
    window.location.replace("/Dashboard");
    return;
  }

  try {
    // 1. Save final progress to DB and mark status = finished
    await fetch(`/projects/complete/${window.currentProjectId}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" }
    });
  } catch (e) {
    console.error("Failed to mark project complete:", e);
    // Still navigate even if the request fails
  }

  // 2. Wipe the resume URL params from browser history so
  //    if anything triggers a reload, DOMContentLoaded won't
  //    see project_id + resume_phase and re-enter the pipeline
  history.replaceState(null, "", "/Dashboard");

  // 3. Navigate to dashboard using replace() so this history
  //    entry is overwritten — back button won't return here
  window.location.replace("/Dashboard");
}
